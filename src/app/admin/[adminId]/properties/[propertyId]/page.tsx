...
export default function EditPropertyPage() {
  ...
  useEffect(() => {
    ...
    const checkAuthAndLoadProperty = async () => {
      ...
      // Admin pode editar qualquer propriedade, não precisa verificar o ID
      ...
      try {
        ...
        // Admin tem permissão para editar qualquer imóvel
        ...
        // Admin não edita dados do proprietário - mantém os originais
      } catch (error) {
  ...
  }, [adminId, propertyId, router, form, dataLoaded]);
  ...
  const onSubmit = async (values: z.infer<typeof propertyFormSchema>) => {
    ...
    try {
      // Obter dados atuais da propriedade para manter o proprietário original
      const currentProperty = await getPropertyById(propertyId);

      // Converter os dados do formulário para o formato esperado
      const propertyData: PropertyFormData = {
        ownerId: currentProperty?.ownerId || "", // Manter o proprietário original

        // Admin não altera dados do proprietário - mantém os originais
        ownerName: currentProperty?.owner?.fullName || "",
        ownerPhone: currentProperty?.owner?.phone || "",
        ownerEmail: currentProperty?.owner?.email || "",
        ownerInstagram: currentProperty?.owner?.instagram || "",
        ownerWebsite: currentProperty?.owner?.website || "",
        ownerProfileImage: currentProperty?.owner?.profileImage || "",
