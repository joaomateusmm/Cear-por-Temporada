...
/**
 * Script final para migrar IDs de proprietários
 * Remove constraints, altera schema, migra dados, recria constraints
 */
async function finalMigrateOwnerIds() {
  console.log("🔄 Iniciando migração final de IDs de proprietários...");
  ...
  try {
    ...
    // 5. Remover constraint de foreign key
    console.log("🔓 Removendo constraint de foreign key...");
    await db.execute(
      sql`ALTER TABLE properties DROP CONSTRAINT IF EXISTS properties_owner_id_owners_id_fk`,
    );
    await db.execute(
      sql`ALTER TABLE properties DROP CONSTRAINT IF EXISTS properties_owner_id_fkey`,
    );

    // 6. Deletar todos os dados existentes (agora pode deletar sem problemas)
    ...
    // 7. Atualizar schema das colunas
    await updateSchemaColumns();

    // 8. Reinserir proprietários com novos IDs
    ...
    // 9. Reinserir propriedades com owner_id atualizados
    ...
    // 10. Recriar constraint de foreign key
    console.log("🔒 Recriando constraint de foreign key...");
    await db.execute(sql`
      ALTER TABLE properties 
      ADD CONSTRAINT properties_owner_id_owners_id_fk 
      FOREIGN KEY (owner_id) REFERENCES owners(id) ON DELETE SET NULL
    `);

    console.log("🎉 Migração final de IDs de proprietários concluída!");
  } catch (error) {
...
async function updateSchemaOnly() {
  console.log("🔄 Atualizando apenas schema (sem dados)...");
...
async function updateSchemaColumns() {
  console.log("🔄 Atualizando tipos das colunas...");
...
export { finalMigrateOwnerIds, verifyMigration };
